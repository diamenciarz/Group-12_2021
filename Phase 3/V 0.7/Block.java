...
public class Block {
    ...
    public void rotate(int xAxis, int yAxis, int zAxis) {
        ...
        for (ParcelPlacement placement : placements) {
            HelperMethods.createParcelPlacementRotation(placement, xAxis, yAxis, zAxis);
        }
        updateDimensions();
     ...
     *         the merge failed due to a collision.
     */
    public boolean mergeWith(ParcelPlacement placement) {
    ...
    private void addParcel(Parcel parcel, Position pos) {
        ...
        parcelsCount[parcel.index]++;
        System.out.println("Added parcel: " + getLetter(parcel.index));
    }

    private void addToInventory(Parcel parcel, Position pos) {
        int[] dim = parcel.dimensions;
        expandInventory(dim, pos);
        setNegativeValuesToZero(pos);
