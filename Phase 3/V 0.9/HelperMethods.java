...
public class HelperMethods {
    ...
    public static Block getBestBlock(int[] emptyVolumeDimensions, Cargo cargo) {
        ArrayList<Block> allBlocks = HelperMethods.copyB(BlockHolder.getBlocks());
        ArrayList<Block> blocksWithEnoughParcelsFor = getBlocksWithEnoughParcels(allBlocks, cargo);
        ArrayList<Block> blocksThatFit = getBlocksThatFit(blocksWithEnoughParcelsFor, emptyVolumeDimensions);
        ...
        BlockHolder.setBlockList(blocksWithEnoughParcelsFor);
        return bestBlock;
    }
    // region Blocks with enough parcels

    private static ArrayList<Block> getBlocksWithEnoughParcels(ArrayList<Block> blocks, Cargo cargo) {
    ...
    private static ArrayList<Block> getBlocksThatFit(ArrayList<Block> blocks, int[] emptyVolumeDimensions) {
        ...
        for (Block block : blocks) {
            returnArrayList.addAll(getRotationsThatFit(block, emptyVolumeDimensions));
        }
        return returnArrayList;
    }

    private static ArrayList<Block> getRotationsThatFit(Block block, int[] emptyVolumeDimensions) {
        ArrayList<Block> returnList = new ArrayList<>();
        boolean fitsNormally = arrayFits(block.dimensions, emptyVolumeDimensions);
        if (fitsNormally) {
            System.out.println("fits normally");
            returnList.add(block);
        }
        ...
        if (fitsRotated2) {
            System.out.println("fits rotation: 1,0,0");
            returnList.add(block.createRotation(1, 0, 0));
        }
        int[] rotated3 = new int[3];
        rotated3[0] = block.dimensions[2];
        rotated3[1] = block.dimensions[0];
        rotated3[2] = block.dimensions[1];
        boolean fitsRotated3 = arrayFits(rotated3, emptyVolumeDimensions);
        if (fitsRotated3) {
            System.out.println("fits rotation: 3,3,0");
            returnList.add(block.createRotation(3, 3, 0));
        }
        ...
        if (fitsRotated4) {
            System.out.println("fits rotation: 0,3,0");
            returnList.add(block.createRotation(0, 3, 0));
        }
        ...
        if (fitsRotated5) {
            System.out.println("fits rotation: 0,0,1");
            returnList.add(block.createRotation(0, 0, 1));
        }
        int[] rotated6 = new int[3];
        rotated6[0] = block.dimensions[1];
        rotated6[1] = block.dimensions[2];
        rotated6[2] = block.dimensions[0];
        boolean fitsRotated6 = arrayFits(rotated6, emptyVolumeDimensions);
        if (fitsRotated6) {
            System.out.println("fits rotation: 1,0,1");
    ...
    // region Display
    public static void displayCargo(Cargo cargo) {
        for (ParcelPlacement placement : cargo.placements) {
            displayParcelPlacement(placement);
        }
    }
    ...
    private static void displayParcelPlacement(ParcelPlacement placement) {
        // gui.addParcelToList(placement.parcel.getShape(), placement.pos.x,
