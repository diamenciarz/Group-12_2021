...
public class Pentago {
    ...
    private static int[][] NewRecursiveSolution(int[][] currentMapState, ArrayList<int[][]> shapesLeftToFitArray) {
        ...
        int[][] mapWithFittedShapes = TryToFitShapesDirectlyToHoles(currentMapStateCopy, shapesLeftToFitArrayCopy);
        if (mapWithFittedShapes != null) {
            // First algorithm
            newMapState = mapWithFittedShapes;

            solutionSteps++;
            boolean hasFittedTheLastShape = shapesLeftToFitArrayCopy.isEmpty();
            if (hasFittedTheLastShape) {
                return newMapState;
            } else {
                int[][] solution = NewRecursiveSolution(newMapState, shapesLeftToFitArrayCopy);
                    ...
                    for (int x : nonFilledRowsAndColumns.get(1)) {
                        ...
                        if (!areTwoFiguresOverlapping(currentMapStateCopy, shapeOnEmptyGrid)) {
                            ...
                            if (AreAllHolesMultiplicationsOfFive(newMapState)) {
                                ...
                                shapesLeftToFitArrayCopy.remove(0);
                                boolean hasFittedTheLastShape = shapesLeftToFitArrayCopy.isEmpty();
                                if (hasFittedTheLastShape) {
                                    return newMapState;
                                } else {
